...
\begin{keyword}
Architecture Recovery \sep Visualization \sep Reverse Engineering \sep Software Tools
\end{keyword}
...
Maintaining a software system implies a continuous effort to keep it up to date with the unanticipated changes in its environment \cite{lehman-softev}. Having therefore a clear and current understanding of the architecture of a system is critical for its maintenance and evolution \cite{Duca09c, pollet-sar}.
...
\fref{flow} presents the general architecture of any architecture recovery tool. Softwarenaut relies on external tools for the first step and provides support for the second and third steps:
...
\item Providing support for collaboration by allowing users to publish and discover previously published architectural views which are indexed by the analyzed system.
...
\paragraph{Structure of the article} We start with describing the particularities of the tool in two of the phases of architecture recovery: information aggregation (\sref{org}) and interactive exploration (\sref{interact}). In \sref{evol} we discuss the integration of evolutionary analysis in Softwarenaut. In \sref{views} we show how sharing architectural views enables collaboration. In \sref{archi} we discuss architectural considerations and in \sref{disc} we discuss the tool-building experience. In \sref{rel} we present related work and in \sref{conc} we conclude and outline future work.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section {Information Abstraction} \label{sec:org}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The fundamental technique that enables abstraction in Softwarenaut is aggregating horizontal relationships between software artifacts along the containment relationships. 
...
\label{sec:high}
Highlighting is a query, like filtering, which does not remove the elements from the view. One can highlight based on pattern matching in the names of the entities (\eg all the classes that nave ``Test'' in their name) or based on structural properties of the code (\eg all the classes that define the Main method). 

When the modules are represented with a modified treemap view the user can see all the classes in the system at once and map various metrics on the color of the classes. In \fref{filpan} the color intensity of a class is proportional to the FANOUT of that class. 
With such a view one can discover the hot spots of the system. In the ArgoUML case study the figure shows two classes which stand out: the \cd{ProjectBrowser} (labeled 2) and the \cd{UmlDiagramRenderer} (labeled 3); the first is the main window of the application and the second is a Factory for figures representing UML elements.
...
The elements of every architectural view in Softwarenaut are nodes representing modules and edges representing relationships between them. To {\em understand} such a view the user needs to understand the role of the nodes and the meaning and reason for the existence of the edges. Metaphorically, if the view was a phrase, the nodes would be the nouns and the edges would be the verbs. One can understand the message only when one has understood all the nouns and the verbs.
...
The modules presented in the architectural view are abstractions of large number of elements. The inspectors for modules have the responsibility of shedding light on the role of a module in a system. There are two types of module inspectors: 
...
\item {\em Inspectors that detail the internals of a module.} They summarize the contents of a module or present them in a more detailed form than is possible in the architectural view. The Class Metrics inspector shows metrics for all the classes contained in a module and its submodules. 
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=\linewidth]{ModuleDetails}
...
Evolutionary filters assume that not all the entities are equally relevant for the task at hand. Different types of evolutionary filters support different goals of analysis. Two such goals, and their corresponding filters are:
...
Traditionally software analysis tools were designed with a {\em analysis-in-one-sitting} attitude  in mind. One was supposed to start the analysis, arrive at some results, and then discard all the data such that if he had to start again, he would start from scratch. In Softwarenaut we want to enable the interruption and continuation of the analysis, and the reuse of the results of the analysis between sessions. Moreover, traditional software analysis never reuses the information that one user has gained about a certain version of the system when another user analyzes the same system. This is {unfortunate} since once published a version of a system never changes and therefore the results of one analysis can {contribute to} further analyses. In Softwarenaut these two problems are addressed with the help of {\em first-class views} and the concept of a {\em Global View Repository}, a service for sharing recovered architectural views.
...
Views are first-class entities in Softwarenaut: one can save and restore any view during the exploration. During an exploration session, each time the analyst encounters a view that presents a relevant perspective on the system he can save it for later reference or for sharing with others. The result of a Softwarenaut analysis session is therefore a {\em set of architectural views}.
...
Once published, a given version of a system never changes. Therefore all the analysis results regarding that version should be made public so that other users can discover previous results and corroborate their findings. In the context of architectural view recovery this means discovering architectural views that others have previously recovered.
...
In the future, the architectural views as saved in the GVR can serve as the basis for monitoring the architectural evolution of the system. After the publication of a new version of the system, Softwarenaut would automatically detect the views affected by the new changes, and present a {\em visual diff} which would make explicit the differences between the state of the system at the moment when the view was created and the latest system version.
...
In the second year of using the tool in the Software Evolution course we decided to have an exploratory study to evaluate the usability and usefulness of the tool. We asked the students to analyze a large software system they have never seen before with the help of Softwarenaut and to produce an architectural report. Our main goal in organizing the experiment was to get feedback on the usability of the tool; our secondary goal was to collect anecdotal evidence on its usefulness for architecture recovery.

Eight users, one PhD and seven master students participated in the experiment. We presented the tool during one hour. Then the users had two hours to perform the tasks presented in \tref{questions}. The case study was ArgoUML. They worked in teams of two, and provided us with a report of their findings. After finalizing the report, they answered a questionnaire on the usability of the tool. 
...
%Type = Article
\bibitem[{Gallagher et~al.(2008)Gallagher, Hatch, and
  Munro}]{gallagher-archvis}
\bibinfo{author}{K.~Gallagher}, \bibinfo{author}{A.~Hatch},
  \bibinfo{author}{M.~Munro},
\newblock \bibinfo{title}{Software architecture visualization: An evaluation
  framework and its application},
\newblock \bibinfo{journal}{IEEE Transactions on Software Engineering}
  \bibinfo{volume}{34} (\bibinfo{year}{2008}) \bibinfo{pages}{260--270}.
%Type = Inproceedings
\bibitem[{Buchsbaum and Westbrook(2000)}]{buchsbaum-hierarchicalgraphs}
\bibinfo{author}{A.~L. Buchsbaum}, \bibinfo{author}{J.~R. Westbrook},
\newblock \bibinfo{title}{Maintaining hierarchical graph views},
\newblock in: \bibinfo{booktitle}{Proceedings of the eleventh annual ACM-SIAM
  symposium on Discrete algorithms}, SODA '00, \bibinfo{publisher}{Society for
  Industrial and Applied Mathematics}, \bibinfo{address}{Philadelphia, PA,
  USA}, \bibinfo{year}{2000}, pp. \bibinfo{pages}{566--575}.
%Type = Article
...
%Type = Inproceedings
\bibitem[{Storey et~al.(2005)Storey, Cubrani\'{c}, and
  German}]{storey-awareness}
\bibinfo{author}{M.-A.~D. Storey}, \bibinfo{author}{D.~Cubrani\'{c}},
...
%Type = Manual
\bibitem[{1471-2000(2000)}]{ieee-arch}
\bibinfo{author}{I.~S. 1471-2000}, \bibinfo{title}{IEEE Std 1471-2000. IEEE
  recommended practice for architectural description of software-intensive
  systems.}, \bibinfo{organization}{IEEE Architecture Working Group},
  \bibinfo{year}{2000}.
%Type = Inproceedings
...
%Type = Phdthesis
\bibitem[{G{\^\i}rba(2005)}]{girba-thesis}
\bibinfo{author}{T.~G{\^\i}rba}, \bibinfo{title}{Modeling History to Understand
  Software Evolution}, Ph.D. thesis, University of Berne,
  \bibinfo{address}{Berne}, \bibinfo{year}{2005}.
%Type = Inproceedings
\bibitem[{Antoniol et~al.(2004)Antoniol, Di~Penta, and
  Merlo}]{antoniol-discontinuities}
\bibinfo{author}{G.~Antoniol}, \bibinfo{author}{M.~Di~Penta},
  \bibinfo{author}{E.~Merlo},
\newblock \bibinfo{title}{An automatic approach to identify class evolution
  discontinuities},
\newblock in: \bibinfo{booktitle}{IWPSE '04: Proceedings of the Principles of
  Software Evolution, 7th International Workshop}, \bibinfo{publisher}{IEEE
  Computer Society}, \bibinfo{address}{Washington, DC, USA},
  \bibinfo{year}{2004}, pp. \bibinfo{pages}{31--40}.
%Type = Phdthesis
...
\newblock \bibinfo{title}{The story of moose: an agile reengineering
  environment},
\newblock \bibinfo{journal}{SIGSOFT Soft.Eng. Notes} \bibinfo{volume}{30}
...
%Type = Article
\bibitem[{Linstead et~al.(2009)Linstead, Bajracharya, Ngo, Rigor, Lopes, and
  Baldi}]{linstead-sourcerer}
\bibinfo{author}{E.~Linstead}, \bibinfo{author}{S.~Bajracharya},
  \bibinfo{author}{T.~Ngo}, \bibinfo{author}{P.~Rigor},
  \bibinfo{author}{C.~Lopes}, \bibinfo{author}{P.~Baldi},
\newblock \bibinfo{title}{Sourcerer: mining and searching internet-scale
  software repositories},
\newblock \bibinfo{journal}{Data Mining and Knowledge Discovery}
  \bibinfo{volume}{18} (\bibinfo{year}{2009}) \bibinfo{pages}{300--336}.
  \bibinfo{note}{10.1007/s10618-008-0118-x}.
%Type = Inproceedings
...
\newblock in: \bibinfo{booktitle}{SoftVis '03: Proceedings of the 2003 ACM
  ...
  \bibinfo{pages}{47--ff}.
%Type = Article
\bibitem[{Sinha et~al.(2006)Sinha, Karger, and Miller}]{sinh-relo}
\bibinfo{author}{V.~Sinha}, \bibinfo{author}{D.~Karger},
  \bibinfo{author}{R.~Miller},
\newblock \bibinfo{title}{Relo: Helping users manage context during interactive
  exploratory visualization of large codebases},
\newblock \bibinfo{journal}{Visual Languages and Human-Centric Computing, 2006.
  VL/HCC 2006. IEEE Symposium on}  (\bibinfo{year}{2006})
  \bibinfo{pages}{187--194}.
%Type = Article
...
%Type = Inproceedings
\bibitem[{Wettel et~al.(2011)Wettel, Lanza, and Robbes}]{wettel-icse11}
\bibinfo{author}{R.~Wettel}, \bibinfo{author}{M.~Lanza},
  \bibinfo{author}{R.~Robbes},
\newblock \bibinfo{title}{Software systems as cities: a controlled experiment},
\newblock in: \bibinfo{booktitle}{Proceeding of the 33rd international
  conference on Software engineering}, ICSE '11, \bibinfo{publisher}{ACM},
  \bibinfo{address}{New York, NY, USA}, \bibinfo{year}{2011}, pp.
  \bibinfo{pages}{551--560}.
%Type = Phdthesis
...
\newblock in: \bibinfo{booktitle}{Proceedings of CSMR 2008 (12th European
  ...
  \bibinfo{pages}{3--12}.
%Type = Inproceedings
\bibitem[{Hattori et~al.(2011)Hattori, D'Ambros, Lanza, and
  Lungu}]{hattori-replay}
\bibinfo{author}{L.~Hattori}, \bibinfo{author}{M.~D'Ambros},
  \bibinfo{author}{M.~Lanza}, \bibinfo{author}{M.~Lungu},
\newblock \bibinfo{title}{Software evolution comprehension: Replay to the
  rescue},
\newblock in: \bibinfo{booktitle}{Proceedings of The 19th IEEE International
  Conference on Program Comprehension}, pp. \bibinfo{pages}{161--170}.
%Type = Inproceedings
