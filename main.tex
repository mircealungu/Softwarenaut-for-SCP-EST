...
The highest level implicit relationship is the one between A and B. 

Problem: Aggregating the relationships across the vertical hierarchy has an $O(n^2)$ order of complexity \cite{buchsbaum-hierarchicalgraphs}. For analyzing non-trivial systems, the on-the-fly computation of the dependencies can hinder interactivity. As a result, one wants to be able to pre-compute the dependencies between the nodes in the hierarchy. The data structure that allows keeping track of the pre-computed dependencies is a {\em hierarchical graph}. \fref{hmodel} presents our implementation of a hierarchical graph model in Softwarenaut.
...
The diagram shows that a HierarchicalGraph contains a Tree which represents the entities in the modeled system and a set of relationships between pairs of elements in this tree.


Entity objects and multiple Relationship objects. Every relationship has two associated entities. 

The Entity class models a tree which using the composite design pattern:

\begin{itemize}

\item {\em Leaf Entities}. A leaf entity is a wrapper around an entity which represents a basic object-oriented programming artifact. They can vary based on the type of analysis performed. An analysis which is quite high-level can use as leaf entities Classes, while an analysis which is more detailed might use methods and instance variables as leaves.

%They are the basic object-oriented programming building blocks used for the structuring of software. The figure illustrates one example of Leaf Entity: the method. Other types of entities can also be represented as leaves (e.g. instance variables, class annotations, etc.).
...
The relationships in a hierarchical graph are of two types:
\begin{itemize}
