...
Eight participants, seven master and one PhD student participated in the experiment. We asked them to perform the tasks presented in \fref{tasks} on ArgoUML. We had them work in teams of two and asked them to provide us with a report of their findings. Our main goal was to learn about usability but we also analyzed their reports. Several of the observations we made are: 
...
None of the students could offer an estimation. Obviously the tool is not supporting such a feature.
\end{itemize}

We also asked the students to fill a post-analysis questionnaire with their evaluations on the tool usability. Most of the students found the tool easy to use and considered that the reports they generated were reliable. They estimated that they got a better understanding of the system by using the tool. 

However, they noted that there were several features that they required from the tool. In the order of the frequency of requests we list the features here: 

\begin{enumerate}
\item User defined filters (all incoming dependencies, all outgoing dependencies, dependencies weaker than...) (3 requests)
\item Arbitrary grouping of items (selected items, classes whose name matches a certain pattern, orphan classes) (3 requests)
\item Undo and Redo operations (2 requests)
\item Selecting edges (multiple edges, all outgoing edges) (2 requests)
\item View persistence (1 request)
\end{enumerate}


Since the time we had performed this study, many of these features are already implemented. Some are still on the backburner.

Performing a real controlled experiment remains as future work.
In the future we plan to perform usability experiments that will evaluate the usability and usefulness of collaboration in architecture recovery and its implementation in the GVR by running some more controlled experiments.
