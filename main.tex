...
\fref{filpan} highlights the relation filtering panel in Softwarenaut in which one can select and combine filters that apply on relationships. The user can also define new filters by writing simple scripts in Smalltalk. Once a new filter is defined, it is immediately available for use. Several filters can be combined to obtain more expressive ones with either the ``and'' or the ``or'' operator. The elements that match the filter can be either ``shown'' or ``hidden''. 
