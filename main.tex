...
\caption{Softwarenaut addresses the last two phases of an extract-abstract-view model}
\flabel{flow}
\end{center}
\end{figure}
...
\on{avoid foobar examples!!!}
\on{Don't rely on colors in diagrams unless you really need to! (use different kinds arrowheads or lines to distinguish arrows, not colors)}
\on{use UML conventions if they exist (use UML arrows, not invented ones, where available)}
}
\flabel{dep-agg}
\end{center}
\end{figure}
...
Aggregating the relationships across the \ugh{vertical} \on{you haven't explained what you mean by "vertical" and "horizontal" --- software has no inherent direction!} hierarchy has an $O(n^2)$ order of complexity \cite{buchsbaum-hierarchicalgraphs}. This means that for non-trivial systems, the on-the-fly computation of the dependencies can make interactive analysis sluggish.
...
To \chg{allow}{enable} interactivity one \ugh{must} pre-compute the dependencies between the nodes in the hierarchy. The data structure that \chg{allows keeping}{is used to keep} \on{Please avoid this construct ("allows doing") -- it is ungrammatical and you use it too often.} track of the pre-computed dependencies is a {\em hierarchical graph}. 

\fref{hmodel} presents a class diagram of Softwarenaut's hierarchical graphs.
...
\on{how about a few words of introduction? what are hierarchical graphs for?  are they the internal data structures used by softwarenaut to model everything? if so, say so.}
...
\caption{Two methods \chg{allow}{enable} the runtime discovery of a detail view}
