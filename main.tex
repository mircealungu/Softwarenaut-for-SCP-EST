...
The Detail panel of Softwarenaut is one of the ways in which one can understand the individual nodes and edges in an architectural view; it presents different detail views depending on the selected element in the exploration view. %There are therefore, two types of detail views: for nodes and for edges. 



\subsubsection {Detail Views for Modules}
They present details about the selected module. \fref{dnode} presents, as an example, the {\em Class Metrics} detail view, a view which presents all the classes contained in a certain module and metrics for them. The set of displayed metrics is customizable. One can navigate to the code of the individual classes. 
